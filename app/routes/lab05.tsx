...
export default function MyTermProject () {
  ...
  const handleClick = (tpId: any, ind: any) => {
    ...
    myOut+= "ระบุข้อมูลอื่น ๆ เพิ่มเติมที่นี่...\n";
    myOut+= "\n-- Thank you --";
  ...
  const termProj = tpSingle.map((tpObj, index) =>
    <div className="grid grid-cols-5 mx-auto border-gray-300" key={index}>
      ...
      <div className="p-4 mb-auto">
        <a href="#" className="bg-indigo-600 text-white rounded-sm hover:bg-indigo-700 px-7 py-2"
  ...
  return (
    <>
      ...
      <div className="w-1/2 grid mx-auto h-100 overflow-hide rounded-2xl border border-gray-300 flex justify-center grid grid-cols-3 gap-2 mb-1">
        <button className="w-50 bg-blue-500 hover:bg-blue-400 text-white font-bold py-2 px-4 border-b-4 border-blue-700 hover:border-blue-500 rounded" onClick={() => handleTeam(null)}>
          [A] All
        </button>
        <button className="w-50 bg-green-500 hover:bg-green-400 text-white font-bold py-2 px-4 border-b-4 border-green-700 hover:border-green-500 rounded" onClick={() => handleTeam(true)}>
          [T] Team
        </button>
        <button className="w-50 bg-yellow-500 hover:bg-yellow-400 text-white font-bold py-2 px-4 border-b-4 border-yellow-700 hover:border-yellow-500 rounded" onClick={() => handleTeam(false)}>
          [S] Single
        </button>
      </div>
